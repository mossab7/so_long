...
void free_map_resources(char **map,int size)
{
    while(size < 0)
        free(map[size--]);
    free(map);
    ft_putendl_fd("error while reading map <check if the path is correct>",stdout);
}
...
int read_map(char *map_path, t_game *game)
{
    ...
    while(1)
    {
        ...
        {
            ft_putendl_fd("error while reading from map",stdout);
            return -1;
        }
        free(tmp);
    }
...
int main(int ac,char **av)
{
    ...
    if(ac != 2)
    {
        ft_putendl_fd("wrong format <./so_long mapXX.ber>",stdout);
        return 0;
    }
    game = (t_game){0};
