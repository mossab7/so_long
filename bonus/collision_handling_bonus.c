...
{
	return (obj1.x_pos + overlap >= obj2.x_pos && obj1.x_pos <= obj2.x_pos
		+ overlap && obj1.y_pos + overlap >= obj2.y_pos
		&& obj1.y_pos <= obj2.y_pos + overlap);
}

int	is_valid_move(t_game *game, int enemy_idx, int new_x, int new_y)
{
	if (is_out_of_bounds(game, new_x, new_y) || is_wall(game, new_x, new_y)
...
{
	return (x < 0 || x >= game->map.width * SCALE || y < 0
		|| y >= game->map.height * SCALE);
}
